...
if (isset($_POST['submit'])) {
    ...
    $code = trim($_POST['code_category'] ?? '');
    $name = trim($_POST['name_category'] ?? '');
   

    if (empty($errors)) {
        $query = "UPDATE tb_category SET code_category='$code', name_category='$name', parent_id=0 WHERE id_category=$id";
        $result = mysqli_query($dbc, $query);
        kt_query($query, $result);
        if ($result) {
            header("Location: responsive_table.php?success=1");
            exit();
        } else {
            $message = "<p class='results1'>Chỉnh sửa không thành công</p>";
        }
    } else {
        $message = "<p class='results1'>Vui lòng nhập đầy đủ thông tin: ".implode(', ', $errors)."</p>";
    }
}

// Lấy dữ liệu category từ DB
...
<form name="frmadd-video" method="post">
    ...
    ?>
    <h2 style="color: red">Chỉnh sửa - loại sản phẩm "<?php echo htmlspecialchars($dong['name_category']); ?>"</h2>
    
    <div class="form-group">
        <label>Mã danh mục</label>
        <input type="text" name="code_category" 
               value="<?php echo isset($_POST['code_category']) ? htmlspecialchars($_POST['code_category']) : htmlspecialchars($dong['code_category']); ?>" 
               class="form-control" placeholder="Mã loại sản phẩm"/>
    </div>

    <div class="form-group">
        <label>Tên danh mục</label>
        <input type="text" name="name_category" 
               value="<?php echo isset($_POST['name_category']) ? htmlspecialchars($_POST['name_category']) : htmlspecialchars($dong['name_category']); ?>" 
               class="form-control" id="name_category" current_id="<?php echo $id; ?>" parent_id="<?php echo $dong['parent_id']; ?>" 
