...
<div class="top-nav clearfix">
    ...
    <ul class="nav pull-right top-menu">
        ...
        <li class="dropdown">
            <a data-toggle="dropdown" class="dropdown-toggle" href="#">
                <img alt="" src="Upload1/nguoi1.png ">
                <span class="username">nhanha23</span>
                <b class="caret"></b>
            </a>
...
<section id="main-content">
  <section class="wrapper">
    ...
    <div class="row">
      <div class="col-md-12">
        <div class="panel panel-default" style="box-shadow:0 6px 18px rgba(0,0,0,0.04);">
          <div class="panel-body">
            
<style>
    .results { color: #009966; font-weight:600; }
    .results1 { color: #FF0000; font-weight:600; }
...
if (isset($_POST['submit'])) {
    $errors = array();
    if (empty($_POST['account'])) $errors[] = 'account';
    else $taikhoan = $_POST['account'];

    if (empty($_POST['name'])) $errors[] = 'name';
    else $hoten = $_POST['name'];

    if (empty($_POST['birthday'])) $errors[] = 'birthday';
    else $birthday = $_POST['birthday'];

    if (empty($_POST['cmnd'])) $errors[] = 'cmnd';
    else $cmnd = $_POST['cmnd'];

    if (empty($_POST['phone'])) $errors[] = 'phone';
    else $dienthoai = $_POST['phone'];

    if (filter_var($_POST['email'], FILTER_VALIDATE_EMAIL)) {
        $email = mysqli_real_escape_string($dbc, $_POST['email']);
    } else $errors[] = 'email';

    if (empty($_POST['address'])) $errors[] = 'address';
    else $diachi = $_POST['address'];
    ...
    if (empty($errors)) {
        ...
        echo $result_in == 1 
            ? "<p class='results'>Chỉnh sửa thành công</p>"
            : "<p class='results1'>Chỉnh sửa không thành công</p>";
    } else {
...
<form method="post">
    ...
    <div class="form-group">
        ...
        <input type="text" name="account" class="form-control" 
               value="<?php echo $hienthi['account_user']; ?>" placeholder="Tài khoản">
        <?php if(isset($errors) && in_array('account',$errors)) echo "<p class='results1'>Bạn hãy nhập Tài khoản</p>"; ?>
    </div>

    <div class="form-group">
        <label>Họ tên</label>
        <input type="text" name="name" class="form-control"
               value="<?php echo $hienthi['name_user']; ?>" placeholder="Họ tên">
        <?php if(isset($errors) && in_array('name',$errors)) echo "<p class='results1'>Bạn hãy nhập họ tên</p>"; ?>
    </div>

    <div class="form-group">
        <label>Ngày sinh</label>
        <input type="date" name="birthday" class="form-control"
               value="<?php echo $hienthi['birthday_user']; ?>">
        <?php if(isset($errors) && in_array('birthday',$errors)) echo "<p class='results1'>Bạn hãy nhập ngày sinh</p>"; ?>
    </div>
    ...
    <div class="form-group">
        ...
        <input type="text" name="cmnd" class="form-control"
               ...
               value="<?php echo $hienthi['cmnd_user']; ?>" placeholder="Chứng minh nhân dân">
        <?php if(isset($errors) && in_array('cmnd',$errors)) echo "<p class='results1'>Bạn hãy nhập CMND</p>"; ?>
    </div>

    <div class="form-group">
        <label>Điện thoại</label>
        <input type="text" name="phone" class="form-control"
               value="<?php echo $hienthi['phonenumber_user']; ?>" placeholder="Điện thoại">
        <?php if(isset($errors) && in_array('phone',$errors)) echo "<p class='results1'>Bạn hãy nhập điện thoại</p>"; ?>
    </div>

    <div class="form-group">
        <label>Email</label>
        <input type="text" name="email" class="form-control"
               value="<?php echo $hienthi['email_user']; ?>" placeholder="Email">
        <?php if(isset($errors) && in_array('email',$errors)) echo "<p class='results1'>Email không hợp lệ</p>"; ?>
    </div>

    <div class="form-group">
        <label>Địa chỉ</label>
        <input type="text" name="address" class="form-control"
               value="<?php echo $hienthi['address_user']; ?>" placeholder="Địa chỉ">
        <?php if(isset($errors) && in_array('address',$errors)) echo "<p class='results1'>Bạn hãy nhập địa chỉ</p>"; ?>
    </div>
    ...
    <div class="form-group">
        ...
        <label class="radio-inline">
            <input type="radio" name="status" value="1"
                <?php echo $hienthi['status_user']==1 ? 'checked' : ''; ?>> 
            <span class="results">Hoạt động</span>
        </label>

        <label class="radio-inline">
            <input type="radio" name="status" value="0"
                <?php echo $hienthi['status_user']==0 ? 'checked' : ''; ?>> 
            <span class="results1">Không hoạt động</span>
        </label>
    </div>
