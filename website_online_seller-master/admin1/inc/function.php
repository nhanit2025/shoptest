...
function check_product($id_product, $size, $quantity_order) {
    ...
    if (!$row || empty($row['size_product'])) {
        return false; // không có dữ liệu size
    }

    // Giải serialize
    $size_product = unserialize($row['size_product']);

    // Nếu unserialize lỗi hoặc không phải array
    if (!is_array($size_product)) {
        return false;
    }
    ...
    if (!array_key_exists($size_key, $size_product)) {
        return false;  // size không tồn tại trong DB
    }

    // Kiểm tra tồn kho
    if ($size_product[$size_key] < $quantity_order) {
