    ...
    <ul class="sub">
        <li><a href="responsive_table.php">Danh sách danh mục</a></li>
                ...
                <li class="sub-menu"><a href="javascript:;"><i class="fa fa-tasks"></i><span>Quản lý đơn hàng</span></a>
                 ...
                 <li><a href="login.php"><i class="fa fa-user"></i><span>Login Page</span></a></li>
    ...
    <div class="row">
      <div class="col-md-12">
        <div class="panel panel-default" style="box-shadow:0 6px 18px rgba(0,0,0,0.04);">
          <div class="panel-heading"><strong>Thống kê sản phẩm bán chạy</strong></div>
            ...
            <div class="table-responsive">
              <table class="table table-bordered table-hover">
                ...
                <tbody>
                  ...
                  // Query lấy top 10 sản phẩm theo tổng số lượng đặt (group by id_product)
                  // Chú ý: biến $dbc phải được tạo trước (include vào file)
                 $query = "SELECT tb_order.id_product, tb_product.code_product, tb_product.name_product,tb_product.price_product,tb_product.saleprice_product, tb_product.image,tb_product.view_product,tb_product.size_product, tb_product.status_product, SUM(tb_order.quantity_product) quantity_product_order, COUNT(tb_order.id_order) total_order FROM tb_order, tb_product WHERE tb_order.id_product = tb_product.id_product GROUP BY tb_order.id_product ORDER BY quantity_product_order DESC LIMIT 10";
                  ...
                  if($result){
                    while ($product = mysqli_fetch_array($result, MYSQLI_ASSOC)) { 
                        $stt++;
                        // Tính tổng số lượng từ kích thước (size_product lưu serialized array)
                        $sl = 0;
                        if(!empty($product['size_product'])){
                          ...
                          if($sizes && is_array($sizes)){
                            ...
                            // nếu không phải serialized -> cố gắng parse số nguyên
                            $sl = is_numeric($product['size_product']) ? (int)$product['size_product'] : $sl;
                          }
                        }
                        // xử lý ảnh: lấy ảnh đầu tiên trong chuỗi (ngăn cách bởi space)
                        
                  ?>
                  <tr>
                    <th><?php echo $stt; ?></th>
                    <td><?php echo htmlspecialchars($product['code_product']); ?></td>
...
foreach ($img_product as $value) {
    if (!empty($value)) {
        echo '<img src="' . $value . '" style="width:60px;height:60px;object-fit:cover;">';
        break;
    }
}
?>
                    ...
                    <td><?php echo number_format($product['price_product']); ?> <strong>VND</strong></td>
                    <td><?php echo number_format($product['saleprice_product']); ?> <strong>VND</strong></td>
                    <td class="text-center"><?php echo (int)$sl; ?></td>
                    <td class="text-center"><?php echo number_format($product['view_product']); ?></td>
                    <td class="text-center"><?php echo number_format($product['total_order']); ?></td>
                    <td><?php echo $product['status_product'] ? "Còn hàng" : "Hết hàng"; ?></td>
                    <td align="center">
                      <a class="link_a" href="edit_product.php?id=<?php echo urlencode($product['id_product']); ?>">
                        <i class="fa fa-eye" title="Xem/Sửa"></i>
                      </a>
                    </td>
                  </tr>
                  <?php 
                    ...
                    // lỗi truy vấn
                    echo '<tr><td colspan="11">Không lấy được dữ liệu.</td></tr>';
                  }
                  ?>
                </tbody>
              </table>
