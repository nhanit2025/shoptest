                ...
                <div class="hidden-sm hidden-xs wap">
                  ...
                  <!--   <div id="pre"><img src="pre-icon.png"></div>
                   ...
                   <ul id="img">
                     ...
                     $array_slider = array_filter(explode(' ', trim($value)));

                     $stt = 0;
                     $total_slider = count($array_slider);
                     foreach ($array_slider as  $value) {
                        ?>
                        <li stt="<?php echo $stt; ?>"><img src="<?php echo $value; ?>" style=""></li>
                        <?php
                        $stt++;
                    }
                    ?>
...
while ($category_c = mysqli_fetch_array($result_category_c, MYSQLI_ASSOC)) {
    ...
    $id_list = lay_id($category_c['id_category']);

    if (empty($id_list)) {
        // Không có con → chỉ lấy chính nó
        $id = $category_c['id_category'];
    } else {
        // Ghép cha + con
        $id = $category_c['id_category'] . "," . $id_list;
    }

    // Lúc này $id mới đúng để đưa vào WHERE IN()
                    ...
                    ?>
                        ...
                        <div class="row">
                            ...
                            <div class="col-xs-12 wp" style="margin-top: 20px">
                                <div class="row width">
                                    <div class="wapper" style="display: flex;
  flex-wrap: wrap;        /* cho phép xuống dòng */
  gap: 30px;              /* khoảng giữa item (hiện bạn đang +30 trong JS) */
  align-items: stretch;">
                                        <?php
    ...
    aspect-ratio: 1 / 1;
    object-fit: cover;
    max-height: 300px;   /* thêm dòng này */
    border-radius: 6px;">
                                                        <?php
